...
class TypographyTest {
    ...
    fun `numeric styles use tabular figures`() {
        listOf(
            "numeric" to bpodcatTypeExtras.numeric,
            "numericLarge" to bpodcatTypeExtras.numericLarge,
        ).forEach { (name, style) ->
    ...
    private fun displayRoles(): List<Pair<String, TextStyle>> = with(bpodcatTypography) {
        listOf(
    ...
    private fun textRoles(): List<Pair<String, TextStyle>> = with(bpodcatTypography) {
        listOf(
